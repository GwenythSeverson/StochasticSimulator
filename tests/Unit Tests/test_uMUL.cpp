...
// Unit tests for the uGEMM Figure 3(a) unipolar uMUL.
//
...
protected:
    ...
    // Tolerance for a uMUL result at N = 1024 on auto width. Measured max error over a 361-point
...
// never on an in_0 zero. This is the single property that makes uMUL correlation-immune, so if
...
TEST_F(UnaryMultiplierTest, EnableFiresExactlyOnInput0Ones) {
    ...
    for (std::size_t i = 0; i < N; ++i) {
        umul.multiply(a[i], b[i]);
    }

    EXPECT_EQ(umul.get_enabled_cycles(), count_ones(a));
    EXPECT_LT(umul.get_enabled_cycles(), N);  // in_0 is not all ones, so some cycles must have held
}
...
TEST_F(UnaryMultiplierTest, OutputIsAlwaysSubsetOfInput0) {
    ...
    std::vector<bool> out = umul_stream(a, b);
...
TEST_F(UnaryMultiplierTest, ZeroOnInput0GivesExactlyZero) {
    std::vector<bool> zeros(N, false);
    std::vector<bool> b = make_stream(0.75, 9);

    EXPECT_DOUBLE_EQ(umul_probability(zeros, b), 0.0);
    EXPECT_EQ(count_ones(umul_stream(zeros, b)), 0u);
}
...
TEST_F(UnaryMultiplierTest, DeterministicAcrossReset) {
    ...
    for (std::size_t i = 0; i < N; ++i) first.push_back(umul.multiply(a[i], b[i]));

    umul.reset();
    for (std::size_t i = 0; i < N; ++i) second.push_back(umul.multiply(a[i], b[i]));

    EXPECT_EQ(first, second);
    EXPECT_EQ(umul.get_enabled_cycles(), count_ones(a));  // not double-counted across the reset
}
...
TEST_F(UnaryMultiplierTest, CounterStartsHalfFull) {
    UnaryMultiplier umul(8);
    EXPECT_EQ(umul.get_entry(), 256u);
    EXPECT_EQ(umul.get_counter_value(), 128u);
    EXPECT_DOUBLE_EQ(umul.get_tracked_probability(), 0.5);
...
TEST_F(UnaryMultiplierTest, CounterIsBinaryAndReachesBothRails) {
    const uint64_t ENTRY = 64;  // width 6
    std::vector<bool> ones(N, true);
    std::vector<bool> zeros(N, false);

    UnaryMultiplier high(6);
...
TEST_F(UnaryMultiplierTest, CounterShiftsEvenWhileEnableIsLow) {
    UnaryMultiplier umul(6);
    for (std::size_t i = 0; i < 64; ++i) {
        umul.multiply(false, true);  // in_0 held LOW the whole time
    }
    EXPECT_EQ(umul.get_enabled_cycles(), 0u);   // RNG never stepped
    EXPECT_EQ(umul.get_counter_value(), 64u);   // ...but the counter still filled
}
...
TEST_F(UnaryMultiplierTest, OperandsAreNotInterchangeable) {
    std::vector<bool> a = make_stream(0.25, 55);
    std::vector<bool> b = make_stream(0.80, 601);

    std::vector<bool> forward = umul_stream(a, b);
    std::vector<bool> reverse = umul_stream(b, a);
...
TEST_F(UnaryMultiplierTest, WidthForStreamLengthFollowsTheSqrtRule) {
    EXPECT_EQ(UnaryMultiplier::width_for_stream_length(256), 5u);
    EXPECT_EQ(UnaryMultiplier::width_for_stream_length(512), 5u);
    EXPECT_EQ(UnaryMultiplier::width_for_stream_length(1024), 6u);
    EXPECT_EQ(UnaryMultiplier::width_for_stream_length(4096), 7u);
    EXPECT_EQ(UnaryMultiplier::width_for_stream_length(16384), 8u);
...
TEST_F(UnaryMultiplierTest, AutoWidthMatchesTheExplicitWidth) {
    std::vector<bool> a = make_stream(0.45, 12);
    std::vector<bool> b = make_stream(0.55, 808);

    unsigned expected_width = UnaryMultiplier::width_for_stream_length(N);
    EXPECT_EQ(umul_stream(a, b), umul_stream(a, b, expected_width));
}

TEST_F(UnaryMultiplierTest, StreamLengthConstructorMatchesSobol) {
    UnaryMultiplier umul(StreamLength::Length_1024, 1);
    EXPECT_EQ(umul.get_width(), 10u);
    EXPECT_EQ(umul.get_entry(), 1024u);
}
...
TEST_F(UnaryMultiplierTest, MultipliesIndependentStreams) {
    ...
    for (double p0 : probs) {
        for (double p1 : probs) {
            ...
            double expect = calculate_probability(a) * calculate_probability(b);
            EXPECT_NEAR(umul_probability(a, b), expect, TOL)
                << "p0=" << p0 << " p1=" << p1;
        }
    }
}
...
// correlated -- and uMUL still multiplies them.
TEST_F(UnaryMultiplierTest, StaysAccurateOnMaximallyCorrelatedStreams) {
...
// the correlation problem went away and uMUL is no longer needed -- worth knowing either way.
TEST_F(UnaryMultiplierTest, PlainAndGateCollapsesToMinWhenCorrelated) {
    ...
    double umul_err = std::fabs(umul_probability(a, b) - product);

    EXPECT_GT(and_err, 0.15);                              // the AND gate is badly wrong
    EXPECT_NEAR(umul_probability(a, b), product, TOL);     // uMUL is not
    EXPECT_GT(and_err, umul_err * 5.0);                    // and by a wide margin
}
...
TEST_F(UnaryMultiplierTest, BeatsThePlainAndGateOnCorrelatedStreams) {
    ...
    for (double p0 : probs) {
        for (double p1 : probs) {
            ...
            double umul_err = std::fabs(umul_probability(a, b) - expect);
            double and_err = std::fabs(and_gate(a, b) - expect);

            EXPECT_LE(umul_err, and_err) << "p0=" << p0 << " p1=" << p1;
        }
    }
}
...
TEST_F(UnaryMultiplierTest, EdgeProbabilitiesPayTheWarmUpCost) {
    std::vector<bool> ones(N, true);
    std::vector<bool> zeros(N, false);

    // Not exact -- but must stay inside the warm-up budget.
    EXPECT_NEAR(umul_probability(ones, ones), 1.0, 0.03);
    EXPECT_NEAR(umul_probability(ones, zeros), 0.0, 0.03);
    EXPECT_LT(umul_probability(ones, ones), 1.0);   // the leak is real, in this direction
    EXPECT_GT(umul_probability(ones, zeros), 0.0);

    // 1 * p passes p straight through.
    std::vector<bool> b = make_stream(0.50, 77);
    EXPECT_NEAR(umul_probability(ones, b), calculate_probability(b), TOL);
    EXPECT_NEAR(umul_probability(b, ones), calculate_probability(b), TOL);
}
...
TEST_F(UnaryMultiplierTest, RejectsMalformedStreams) {
    std::vector<bool> two = {true, false};
    std::vector<bool> three = {true, false, true};
    std::vector<bool> empty;

    EXPECT_THROW(umul_stream(two, three), std::invalid_argument);
    EXPECT_THROW(umul_stream(empty, empty), std::invalid_argument);
    EXPECT_THROW(umul_probability(two, three), std::invalid_argument);
}

TEST_F(UnaryMultiplierTest, RejectsOutOfRangeWidth) {
    ...
    EXPECT_NO_THROW((void)UnaryMultiplier(1));
    EXPECT_NO_THROW((void)UnaryMultiplier(UnaryMultiplier::MAX_WIDTH));
}
