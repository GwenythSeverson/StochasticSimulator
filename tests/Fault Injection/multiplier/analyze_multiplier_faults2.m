%% Stochastic Computing Multiplier Exhaustive Fault-Injection Analysis
% This script parses the exhaustive 32-bit fault injection CSV, generates
% standard/updated degradation graphs, visualizes the precision error vs
% bit-flip error, and saves the results to a MATLAB .mat workspace file.

clear; clc; close all;

csvFile = '32bit_exhaustive_multiplier_trials.csv';
if ~isfile(csvFile)
    error('CRITICAL: %s not found. Please run the C++ simulator first.', csvFile);
end

%% 1. Ingest Data via Text Scanning (High Performance)
fprintf('Ingesting exhaustive CSV file... ');
tic;
...
matFile = 'Stochastic_Multiplier_Exhaustive_Data.mat';
fprintf('Saving workspace variables to %s... ', matFile);
save(matFile);
fprintf('Done!\n');

%% 3. Setup Figure Layout (3x3 Grid)
figure('Name', 'Stochastic Multiplier Exhaustive Fault Analysis Suite', ...
       'Units', 'Normalized', 'Position', [0.05, 0.05, 0.9, 0.9]);
t = tiledlayout(3, 3, 'TileSpacing', 'Compact', 'Padding', 'Compact');
title(t, '32-Bit Exhaustive Stochastic Multiplier Fault Injection Suite', ...
...
fig = gcf; 
exportgraphics(fig, 'Stochastic_Multiplier_Exhaustive_Fault_Analysis.png', 'Resolution', 300, 'BackgroundColor', 'current');
