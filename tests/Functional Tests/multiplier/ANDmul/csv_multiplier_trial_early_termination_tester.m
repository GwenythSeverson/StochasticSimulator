...
%% 1. CONFIGURATION
%% ------------------------------------------------------------------
% 1. Walk up from this script until we reach the folder that contains 'keepdata'.
%    This used to count a fixed two '..' steps, which broke as soon as the file moved: it now
%    lives four levels deep (tests/Functional Tests/multiplier/ANDmul/), so two steps landed on
%    'Functional Tests' and dataRootDir pointed at a folder that does not exist. Searching for a
%    known landmark instead survives any future reshuffle.
...
% 2. FORCE Windows/OneDrive to resolve the path into a true absolute path
[status, info] = fileattrib(repoRoot);
if ~status
    error("Could not resolve repository root directory structure.");
end
repoRoot = info.Name; % Clean absolute path to StochasticSimulator/

% 3. Step down into the target data directories
